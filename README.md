...
文档 §4.2 定义了 CAPABILITY 命令（管理端查询代理端能力），文档说返回"支持的模型、模式、帧长、速率和统计字段"，但没有给出具体字段名。

如果管理端先于代理端写好，它期待的字段名可能和这边不一致（比如 `"supported_models"` vs `"models"`）。

**建议**：先按文档命名风格写（全小写+下划线），联调时不对再改，成本很低。**优先级：🟡**

---

### 2. Markov / Regression 参数的精确 JSON 格式
...
```json
// transition_matrix 是二维数组还是扁平？
[[0.7, 0.2, 0.1], [0.2, 0.6, 0.2], [0.1, 0.3, 0.6]]  // 猜测 A
[0.7, 0.2, 0.1, 0.2, 0.6, 0.2, 0.1, 0.3, 0.6]          // 猜测 B

// states 里速率字段名叫什么？
{"name": "low", "rate_mbps": 10}  // 还是 "rate"? "target_rate"?
```
...
```json
// sine 的结构
{"amplitude": 20, "period_s": 60, "phase": 0}
// 还是 {"amp": 20, "period": 60, "phase_rad": 0} ？
```

**为什么重要**：如果字段名猜错，`config.get("amplitude", 0)` 静默返回 0，模型不报错但测试数据全错，极难排查。

**建议**：这是最需要确认的问题。如果师兄说"还没定"，就按目前代码里的实现定义，让管理端跟你对齐。**优先级：🔴**

---

### 3. TCP 下流量模型语义

文档 §5.2 说 transport 可以是 tcp 或 udp，默认 udp。六种流量模型都基于"发包速率"设计（CBR = 等间隔发包）。但 TCP 有自己的拥塞控制（慢启动、拥塞避免、Nagle 算法），不会按指定间隔发包。

**两种可能**：

| 语义 | 做法 | 流量模型还有意义吗 |
|---|---|---|
| 硬限速 | 写一次 → 等 TCP 确认 → 算实际速率 → 补 sleep | 勉强 |
| 尽力而为 | 只管往 socket 写，不管 TCP 怎么发 | CBR 形同虚设 |

**建议**：问师兄一期是否先只支持 UDP，TCP 后续再说。**优先级：🟡**
...
```json
"packet_size": 200,
"payload_pattern": "hex:aabb"
```

payload_len = 200 - 40 = 160 字节。`hex:aabb` 模板只有 2 字节。

| 理解 | 结果 |
|---|---|
| 循环填充 | `aa bb aa bb aa bb ...` 共 160 字节 ✅ 当前实现 |
| 严格长度 | `aa bb` 仅 2 字节，剩余 158 字节填 0 或报错 |

**建议**：循环填充更合理，按此实现即可，优先级低。**优先级：🟢**

---

### 5. `report_interval_ms` 是否一期实现

文档 §5.1 标注为"预留的进度统计周期"。

| 理解 | 做法 |
|---|---|
| 协议预留字段，功能未定义 | 一期不实现，只校验范围 |
| 功能已定义但一期不做 | 校验通过但忽略 |

当前代码只做了范围校验（100~60000），不使用该值。**优先级：🟢**
